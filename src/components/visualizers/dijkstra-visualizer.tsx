...
export default function DijkstraVisualizer() {
  ...
  const initializeGrid = useCallback(() => {
    ...
    while (
      newEndNode.row === newStartNode.row &&
      newEndNode.col === newStartNode.col
    ) {
      newEndNode = {
        row: Math.floor(Math.random() * GRID_SIZE),
        col: Math.floor(Math.random() * GRID_SIZE),
  ...
  const dijkstraSearch = async () => {
    ...
    const resetGrid = grid.map((row) =>
      row.map((n) => ({ ...n, distance: Infinity, parent: null }))
    );
    setGrid(resetGrid);

    const start = resetGrid[startNode.row][startNode.col];
    const end = resetGrid[endNode.row][endNode.col];

    start.distance = 0;
    const openSetLocal: Node[] = [start];
    const closedSetLocal: Node[] = [];

    while (openSetLocal.length > 0) {
      // Pick node with smallest distance
      let lowestIndex = 0;
      for (let i = 1; i < openSetLocal.length; i++) {
        if (openSetLocal[i].distance < openSetLocal[lowestIndex].distance) {
          lowestIndex = i;
        }
      }
      ...
      const neighbors = getNeighbors(currentNode, resetGrid);
      for (const neighbor of neighbors) {
        ...
        const tentative = currentNode.distance + 1; // unit cost grid
  ...
  const getBoxClass = (node: Node) => {
    ...
    if (closedSet.some((n) => n.row === node.row && n.col === node.col))
      return "bg-blue-300";
    if (openSet.some((n) => n.row === node.row && n.col === node.col))
      return "bg-blue-200";
  ...
  return (
    <div className="flex flex-col items-center">
        ...
        <div className="w-[min(95vw,80vh)]">
          ...
          >
            {grid.flat().map((node, idx) => (
              <div
                key={idx}
                className={`aspect-square w-full border border-gray-200 ${getBoxClass(
                  node
                )}`}
      ...
      <div className="flex justify-center gap-4 mt-4">
        <Button onClick={dijkstraSearch} disabled={isSearching}>
          Start Dijkstra
        </Button>
